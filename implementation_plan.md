...
ระบบนี้จะพัฒนาด้วย **Next.js (App Router)** โดยนำเทคโนโลยี **Gemini API** มาใช้จำแนกสายพันธุ์สัตว์จากรูปภาพ และมีช่องทางการใช้งาน 2ช่องทางหลัก:
1. **Web Application**: เว็บแอปพลิเคชันรูปแบบพรีเมียม (Premium Dashboard) รองรับการอัปโหลดไฟล์รูปภาพหรือเปิดกล้องถ่ายภาพ มีการแสดงข้อมูลอย่างละเอียด (Scientific Name, Classification, Habitat, Fun Facts, etc.) และเก็บบันทึกประวัติการสแกนผ่าน LocalStorage
2. **LINE Bot Integration**: ช่องทาง LINE Chatbot เมื่อผู้ใช้ส่งรูปภาพเข้ามา LINE Webhook จะส่งรูปไปให้ Gemini API เพื่อวิเคราะห์ และตอบกลับข้อมูลสรุปของสัตว์ชนิดนั้นๆ ในรูปแบบภาษาไทยที่อ่านง่ายและจัดแต่งสวยงาม
...
> [!NOTE]
> 1. **เวอร์ชันของ Gemini Model**: ในการพัฒนาเบื้องต้น เราจะเลือกใช้ `gemini-2.5-flash` เนื่องจากทำงานได้รวดเร็ว รองรับการอ่านภาพ (Multimodal) และมีประสิทธิภาพสูง
> 2. **วิธีการทดสอบ LINE webhook**: คุณมี ngrok หรือช่องทางอื่นในการทำ HTTPS tunnel อยู่แล้วหรือไม่? (ถ้าไม่มี เราจะแถมคำอธิบายวิธีการติดตั้งและใช้งานง่ายๆ ไว้ในหน้า Dashboard เพื่อความสะดวก)

---

## Proposed Changes

การสร้างและพัฒนาจะแบ่งออกเป็นไฟล์ต่างๆ ดังนี้:

### 1. Library & Integration Layer

#### [NEW] [gemini.ts](file:///c:/Users/chavalit/my-animal2/lib/gemini.ts)
- สร้างฟังก์ชันเชื่อมต่อกับ `@google/genai` (หรือใช้ Node-fetch ยิง REST API ของ Gemini โดยตรงเพื่อลดการพึ่งพารูปแบบ Library ที่อาจเปลี่ยนแปลงบ่อย)
- รองรับการรับข้อมูล Image Base64 หรือ Buffer
- กำหนด JSON Schema สำหรับการสแกนผ่าน Web (เพื่อให้แสดงผล UI สวยงามและมีโครงสร้างข้อมูลที่ชัดเจน)
- กำหนด Prompt สำหรับการสแกนผ่าน LINE (เพื่อให้ตอบข้อความสั้นกระชับ จัดหน้าตาด้วยอิโมจิ)

#### [NEW] [line.ts](file:///c:/Users/chavalit/my-animal2/lib/line.ts)
...
#### [NEW] [route.ts (Recognize API)](file:///c:/Users/chavalit/my-animal2/app/api/recognize/route.ts)
...
- ส่งคืนผลลัพธ์พร้อม Metadata ให้หน้าบ้าน

#### [NEW] [route.ts (LINE Webhook)](file:///c:/Users/chavalit/my-animal2/app/api/line-webhook/route.ts)
...
#### [NEW] [env.template](file:///c:/Users/chavalit/my-animal2/.env.template)
- ตัวอย่างไฟล์ตั้งค่าตัวแปรสภาพแวดล้อม (Environment Variables)
  - `GEMINI_API_KEY`
  - `LINE_CHANNEL_SECRET`
  - `LINE_CHANNEL_ACCESS_TOKEN`

#### [MODIFY] [page.tsx](file:///c:/Users/chavalit/my-animal2/app/page.tsx)
